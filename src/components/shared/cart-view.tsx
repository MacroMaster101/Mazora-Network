  ...
  if (count === 0) {
    return (
      <div className="glass flex flex-col items-center px-6 py-16 text-center">
        ...
        <p className="mt-1 max-w-sm text-sm text-muted">Browse ranks, cosmetics and crate keys to support the network.</p>
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.slug} className="panel flex items-center gap-4 p-4">
            <span className="h-14 w-14 shrink-0 rounded-lg border border-line" style={{ backgroundImage: coverGradient(item.accent) }} />
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => setQty(item.slug, item.qty - 1)} className="grid h-8 w-8 place-items-center rounded-lg border border-line-strong text-muted hover:text-ink" aria-label="Decrease quantity">
                <Minus size={14} />
              </button>
              <span className="telemetry w-8 text-center font-semibold">{item.qty}</span>
              <button onClick={() => setQty(item.slug, item.qty + 1)} className="grid h-8 w-8 place-items-center rounded-lg border border-line-strong text-muted hover:text-ink" aria-label="Increase quantity">
                <Plus size={14} />
              </button>
            </div>
            <span className="telemetry w-20 text-right font-semibold">{usd(item.price * item.qty)}</span>
            <button onClick={() => remove(item.slug)} className="text-muted hover:text-danger" aria-label={`Remove ${item.name}`}>
              <Trash2 size={16} />
            </button>
          </div>
        ))}
      ...
      <div className="glass h-fit p-6 lg:sticky lg:top-24">
        ...
        <button
          onClick={() => toast("Payment integration is coming soon — no charge was made.", "info")}
          className="btn btn-primary mt-5 w-full"
        >
          <Lock size={15} /> Checkout
        </button>
        <p className="mt-3 text-center text-xs text-muted">
          Payments aren&apos;t live in this preview. Checkout is built to plug into a real provider later.
        </p>
      </div>
    </div>
  );
