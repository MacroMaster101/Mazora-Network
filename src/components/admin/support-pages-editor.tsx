          ...
          {filtered.map(({ card, index }) => (
            ...
            >
              <div className={`flex min-w-0 ${view === "cards" ? "items-start" : "flex-1 items-center"} gap-3`}>
                ...
                {view === "rows" && <span className={`hidden shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-black uppercase sm:inline-flex ${card.enabled ? "border-emerald-500/25 bg-emerald-500/10 text-emerald-700 dark:text-emerald-500" : "border-line bg-ink/5 text-muted"}`}>{card.enabled ? "Visible" : "Hidden"}</span>}
              </div>
              ...
              <div className={`flex items-center gap-2 ${view === "cards" ? "mt-4 border-t border-line/60 pt-3" : "ml-3 shrink-0"}`}>
                {view === "cards" && <span className={`mr-auto rounded-full border px-2.5 py-1 text-[10px] font-black uppercase ${card.enabled ? "border-emerald-500/25 bg-emerald-500/10 text-emerald-700 dark:text-emerald-500" : "border-line bg-ink/5 text-muted"}`}>{card.enabled ? "Visible" : "Hidden"}</span>}
