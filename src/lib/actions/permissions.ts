...
import { ALWAYS_ALLOWED, getGalleryPermissions, getNewsPermissions, GALLERY_PERMISSION_KEY, NEWS_PERMISSION_KEY } from "@/lib/auth/permissions";
...
export async function saveNewsPermissionsAction(formData: FormData): Promise<PermissionActionResult> {
  ...
  return { ok: true, message: "News permissions saved." };
}

/** Only the owner tier may change who can manage gallery screenshots. */
export async function saveGalleryPermissionsAction(formData: FormData): Promise<PermissionActionResult> {
  const session = await getSession();
  if (!session || !hasAtLeast(session.role, "owner")) {
    return { ok: false, message: "Only owners can change permissions." };
  }
  ...
  const before = await getGalleryPermissions();
  const value = { roles, userIds };

  await db
    .insert(schema.siteSettings)
    .values({ settingKey: GALLERY_PERMISSION_KEY, settingValue: value })
    .onConflictDoUpdate({
      target: schema.siteSettings.settingKey,
      set: { settingValue: value, updatedAt: new Date() },
    });
  ...
  await db.insert(schema.auditLogs).values({
    action: "gallery.permissions.update",
    targetType: "setting",
    targetId: GALLERY_PERMISSION_KEY,
    metadata: { before, after: value, by: session.username },
  });

  revalidatePath("/admin/permissions");
  revalidatePath("/admin/gallery");
  return { ok: true, message: "Gallery permissions saved." };
}
...
export async function readGalleryPermissions() {
